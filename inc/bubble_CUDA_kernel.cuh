...
static __host__ __device__ void solveRayleighPlesset(float * , float * , float * , float * , const float * , const float * , float * , float *, bub_params_t);	// Solves the Rayleigh Plesset equation
static __host__ __device__ float solveOmegaN(singlecomplex *, float, float, bub_params_t);	// Solves for omega_N
static __host__ __device__ singlecomplex Alpha(float, float, float);	// Calculates alpha_N
static __host__ __device__ singlecomplex Upsilon(singlecomplex , float);	// Calculates Upsilon
static __host__ __device__ singlecomplex solveLp(singlecomplex, float);	// Calculates Lp_N
static __host__ __device__ float solvePG(float, float, float, float, float, singlecomplex, bub_params_t);	// Calculates PG
...
static __inline__ __host__ __device__ float epsilon(float val){
	return 2.22044604925031308e-016;
	//return 2.0e-16;
	//return 1.0e-10;
}

// Implements hyperbolic cotangent for float precision complex variables
static __inline__ __host__ __device__ singlecomplex complexcoth(singlecomplex z){
	float x = tanhf(z.real()), y = tanf(z.imag());
	return make_singlecomplex(1.0, x * y)/make_singlecomplex(x,y);
}

/* Reduced Order Bubble Dynamics Modelling Functions */

// Rayleigh Plesset solver
__inline__ __host__ __device__ void solveRayleighPlesset(float * Rt, float *Rp, float * Rn, float * d1R, const float * PG, const float * PL, float * dt, float * remain, bub_params_t bub_params){
	float 	Rm, d2R, 	// Temporary radius variables
	...
	*Rn = *Rp;
	...
	dtm = *dt;
	dtp = fminf(1.01 * (*dt), bub_params.dt0);
	
	if (fabsf(*d1R) > epsilon(fabsf(*d1R))){
		dtp = min(dtp, 0.01 * (*Rn)/abs(*d1R));
	}
	if (fabsf(d2R) > epsilon(fabsf(d2R))){
		dtp = fminf(dtp, sqrtf(0.01 * (*Rn)/fabsf(d2R)));
	}
...
__inline__ __host__ __device__ float solveOmegaN(singlecomplex * alpha_N, float PG, float R, bub_params_t bub_params){
	float R2 = R * R;
	float coef1 = (bub_params.PG0 * bub_params.R03)/(PG * R * R2);
	float coef2 = PG / (bub_params.rho * R2);
	float coef3 = 4.0 / (bub_params.rho * bub_params.rho * R2 * R2);
	float value1 = -2.0 * bub_params.sig/(bub_params.rho * R2 * R);

	float omega_N;

	// Zeroeth step
	singlecomplex Upsilon_N = make_singlecomplex(3.0*bub_params.gam, 0.0) * coef1;
	float mu_eff = bub_params.mu;
	float eta = Upsilon_N.real() * coef2 + value1 - mu_eff * mu_eff * coef3;

	omega_N = sqrtf(fmaxf(eta, 1.0e-6*epsilon(eta)));
...
// returns alpha_N
static __inline__ __host__ __device__ singlecomplex Alpha(float R, float omega_N, float coeff_alpha){
	return make_singlecomplex(1.0, 1.0) * sqrtf(coeff_alpha * (omega_N) / (R));
}

// returns Upsilon_N
static __inline__ __host__ __device__ singlecomplex Upsilon(singlecomplex a, float gam){
	singlecomplex ctmp;
	if (a.abs() > 1.0e-2){
...
// returns Lp_N
static __inline__ __host__ __device__ singlecomplex solveLp(singlecomplex a, float R){
	singlecomplex ctmp;
	if(a.abs() > 1.0e-1){
...
// returns PG
__inline__ __host__ __device__ float solvePG(float PGn, float Rp, float Rn, float omega_N, float dt, singlecomplex Lp_N, bub_params_t bub_params){
	float 	Lp_NR = Lp_N.real() / (Lp_N.abs() * Lp_N.abs()),
		Lp_NI = Lp_N.imag() / (Lp_N.abs() * Lp_N.abs()),
		R = 0.5 * (Rp + Rn);

	float	c0 = dt*3.0*(bub_params.gam-1.0) * 2.0/(Rp + Rn) * bub_params.K0 * bub_params.T0/(bub_params.PG0 * bub_params.R03),
...
__global__ void BubbleRadiusKernel(int * max_iter){
	...
	float PGn, PL, Rt, omega_N;
	float dTdr_R, SumHeat, SumVis;
	singlecomplex alpha_N, Lp_N;

	float Rp, Rn, d1Rp, PGp, dt_L, remain, PC0, PC1, PC2, time;
