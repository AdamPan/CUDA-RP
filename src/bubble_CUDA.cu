...
void display(double data[], int xdim, int ydim, string msg){
	printf("%s\n", msg);
	for (double j = 0; j <= ydim - 1; j+= (ydim-1)/a){
		if (!j){
			for (double i = 0; i <= xdim - 1; i += (xdim - 1)/a){
				printf("\t(%i)\t",(int)i);
			}
			printf("\n");
		}
...
int solve_bubbles(	array_index_t	*array_index,
	...
	while(	((sim_params->NSTEPMAX != 0) && (nstep < sim_params->NSTEPMAX)) ||
		...
		if(calculate_properties(rho_l_width, rho_m_width, c_sl_width, C_pm_width, f_g_width, T_width, stream, stop)){exit(EXIT_FAILURE);}
		...
		if((((int)nstep) % ((int)sim_params->DATA_SAVE) == 0)){
			...
			if (sim_params->NSTEPMAX != 0){
				...
				printf("resimax = %4.2E\n\n",resimax);
				if(debug->fg){display(mixture_h.fg, i2m, j2m, "fg Grid");}
				if(debug->p0){display(mixture_h.p0, i1m, j1m, "p0 Grid");}
				if(debug->T){display(mixture_h.T, i2m, j2m, "T Grid");}
				if(debug->vxy){
					display(mixture_h.vx, i2n, j2m, "vx Grid");
					display(mixture_h.vy, i2m, j2n, "vy Grid");
				}
				printf("\n\n");
			#endif
		}
